# @sagifire/di-container - Developer Guide (JavaScript)
...
4.  **Asynchronous Support:**
    *   The container allows for asynchronous object creation, which is useful when working with promises or asynchronous factories. **Factories can be `async` functions.**
...
// Assuming these are JavaScript modules
...
container.register({
  ...
  'config': asFactory(configFactory), // configFactory can be an async function
...
*   ### Factory - `asFactory()`
    ...
    ```
    Used for registering an object factory. A factory is a function that creates a component object and **can be asynchronous** (return a Promise). The factory can register its dependencies like a function but must always return a component object (or a Promise resolving to one).
...
Example:
```javascript
// Assuming componentClass and randomFactory are defined elsewhere
container.register({
    'component': asClass(componentClass, ['config', 'log'], LIFETIME_SINGLETON),
    'randomObject': asFactory(randomFactory, LIFETIME_DYNAMIC, ['component']) // randomFactory can be async
});
```
...
// Assuming loadConfigFile is an async function returning a Promise
...
import myContainer from './my-container.js'; // Assuming this exports your configured container
...
class BaseClass {
  ...
  constructor(deps) {
    this.deps = deps;
    this.deps.log('BaseClass constructed');
  }
}

class ChildClass extends BaseClass {
  // Inherit and add dependencies
  static _deps = BaseClass._deps.concat([
    'config'
  ]);

  constructor(deps) {
    super(deps); // Call BaseClass constructor
    this.deps.log('ChildClass constructed');
